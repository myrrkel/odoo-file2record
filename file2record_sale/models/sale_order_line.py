...
class SaleOrderLine(models.Model):
        ...
        for values in values_list:
            if values.get('name') and values.get('product_template_id'):
                ...
                if values.get('product_template_id'):
                    values['product_template_id'] = self.env['product.template']._find_or_create_many2one_record(
                        values.get('product_template_id'))
        res = super(SaleOrderLine, self)._create_one2many_record(values_list)
        for values in res:
    ...
    def _get_fields_description(self, json_dict):
        res = super(SaleOrderLine, self)._get_fields_description(json_dict)
